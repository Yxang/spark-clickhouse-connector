...
import xenon.clickhouse.cityhash.{CityHash_v1_0_2, UInt128}
...
object CityHash64 extends MultiArgsHash {
  ...
  override def invokeBase(value: UTF8String): Long = {
    ...
    val buffered_data = convertToByteBuf(data).array()
    println("cityhash invoke", value, data.map("%02X".format(_)).mkString, buffered_data.map("%02X".format(_)).mkString)
    CityHash_v1_0_2.CityHash64(convertToByteBuf(data), 0, data.length)
  }

  override def combineHashes(v1: Long, v2: Long): Long = {
    println("cityhash combine", v1, v2)
    CityHash_v1_0_2.Hash128to64(new UInt128(v1, v2))
  }
}
